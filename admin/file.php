                    ...
                    <div class="span12">
                        <div class="hero-unit-3">
                            ...
                            <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                                ...
                                <tbody>
                                    <?php
                                    $query = mysqli_query($con,"select * from files") or die(mysqli_error());
                                    while ($row = mysqli_fetch_array($query)) {
                                        $file_id = $row['file_id'];
                                        $teacher_id = $row['teacher_id'];
                                        $class_id = $row['class_id'];

                                        $teacher_query = mysqli_query($con,"select * from teacher where teacher_id='$teacher_id'") or die(mysqli_error());
                                        $teacher_row = mysqli_fetch_array($teacher_query);

                                        $class_query = mysqli_query($con,"select * from class where class_id='$class_id'") or die(mysqli_error());
