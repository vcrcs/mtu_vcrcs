...
<div id="long" class="modal fade modal-scroll" tabindex="-1" data-replace="true" style="margin-top:190px; margin-left:700px;">
				...
				if (isset($_POST['send'])){		
									 ...
									 if($email==$email_verify){
										 mysqli_query($con,"insert into mail(date,send_from,send_to,subject,content,attachment,status_from,status_to) values('$date','$email_come','$email','$subject','$message','"."$image_name"."','$status_from','$status_to')")or die(mysql_error());
				...
				if (isset($_POST['draft'])){		
										...
										$sel=mysqli_fetch_array(mysqli_query("SELECT * from Account where user_id='$session_id'"));
										$email_come=$sel['e_mail'];
										$image = $_FILES['image']['tmp_name'];
										$image_name = $_FILES['image']['name'];
										$image_type = $_FILES['image']['type'];
										...
										mysqli_query("insert into mail(date,send_from,send_to,subject,content,attachment,status_from,status_to) values('$date','$email_come','$email','$subject','$message','"."$image_name"."','$status_from','$status_to')")or die(mysql_error());
											move_uploaded_file($image,"../Uploads/".$image_name);
											echo '<div class="alert alert-dismissable alert-success" style="margin-top:-20px;">';
												echo '<strong>'.'<center>'."Your Message is sent successfully! ".'</center>'.'<strong>';
												echo '</div>';
